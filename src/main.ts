...
console.log('🚀 plans-mfe: Starting web component initialization...');

createApplication(appConfig)
  .then(async (appRef) => {
    console.log('✅ plans-mfe: Angular application created, initializing router...');

    const router = appRef.injector.get(Router);

    // Manually navigate to the default route inside the microfrontend context.
    try {
    ...
    console.log('✅ plans-mfe: Creating <plans-mfe-root> custom element...');
  ...
  .catch((err) => {
    console.error('❌ plans-mfe: Error initializing web component:', err);
    if (err.stack) {
